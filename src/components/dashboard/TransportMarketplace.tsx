...
export function TransportMarketplace({}: TransportMarketplaceProps) {
  ...
  const [pickup, setPickup] = useState('IOCL Terminal, Noida');
  const [delivery, setDelivery] = useState('Primary Industrial Hub Site A, Ghaziabad');
  const [fuelType, setFuelType] = useState('HSD');
  const [quantityKl, setQuantityKl] = useState(10);
  const [requiredDate, setRequiredDate] = useState(new Date(Date.now() + 24*60*60*1000).toISOString().split('T')[0]);
  ...
  const handleBookQuote = async (quoteId: string) => {
    ...
    try {
      ...
      if (res.ok && json.success) {
        ...
        setQuotes([]);
        setActiveSubTab('active_shipments');
        // Fetch details of the new booking
        handleSelectBooking(json.booking);
      } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 bg-white p-4 rounded-xl border border-slate-100">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 bg-blue-100 text-blue-600 rounded-xl flex items-center justify-center font-bold">
            <Truck className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-xl font-bold text-slate-900">Transport Logistics Marketplace</h2>
            <p className="text-xs text-slate-500">Procure verified tankers and manage fuel deliveries</p>
          </div>
        </div>

        <div className="flex bg-slate-100 p-1 rounded-lg">
          <button
            onClick={() => { setActiveSubTab('requests'); setSelectedRequest(null); }}
            className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${
              activeSubTab === 'requests' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
            My Requests
          </button>
          <button
            onClick={() => { setActiveSubTab('active_shipments'); setSelectedBooking(null); }}
            className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${
              activeSubTab === 'active_shipments' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
            Booked Shipments
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all flex items-center gap-1 ${
              activeSubTab === 'create_request' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
                  ...
                  {requests.map((req) => (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center text-[10px] text-slate-500 mt-2">
                        <span>{req.quantityKl} KL &bull; {req.fuelType}</span>
                            ...
                            <div className="text-left sm:text-right flex flex-row sm:flex-col justify-between sm:justify-start items-center sm:items-end gap-2 border-t sm:border-t-0 border-slate-100 pt-3 sm:pt-0">
                              ...
                              {selectedRequest.status !== 'BOOKED' ? (
                                <button
                                  ...
                                  className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-1.5 px-3 rounded-lg text-xs transition-colors"
                                >
                                  Book Transporter
                                </button>
                              ) : isBooked ? (
        ...
        <Card className="max-w-2xl mx-auto">
            ...
            {wizardStep === 2 && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Quantity (KL - Kilolitres)</label>
                    <input
                      type="number"
                      value={quantityKl}
                      onChange={(e) => setQuantityKl(Math.max(1, parseInt(e.target.value) || 0))}
                      className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-slate-900 text-sm font-semibold outline-none focus:border-blue-500 focus:bg-white transition-all"
                    />
                  </div>
                </div>
            ...
            {wizardStep === 3 && (
              <div className="space-y-4 bg-slate-50 p-4 rounded-xl border border-slate-200/60">
                ...
                <div className="space-y-2 text-xs text-slate-700">
                  ...
                  <div className="flex justify-between"><span className="font-semibold">Product Type & Qty:</span><span>{quantityKl} KL ({fuelType})</span></div>
                  <div className="flex justify-between"><span className="font-semibold">Required Date:</span><span>{new Date(requiredDate).toLocaleDateString('en-IN')}</span></div>
                </div>
          ...
          <div className="flex justify-between border-t border-slate-100 pt-6 mt-6">
            ...
            {wizardStep < 3 ? (
              <button
                onClick={() => setWizardStep(prev => prev + 1)}
                className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-lg text-xs transition-colors flex items-center gap-1"
              >
                Next Details
              ...
              <button
                ...
                className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-lg text-xs transition-colors"
              >
                Submit Request
              </button>
            )}
